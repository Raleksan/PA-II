...
int main()
{
    ...
    uniform_real_distribution<double> interval(-20, 20);
    ...
    for (int i = 0; i < n; i++)
    {
        bf1 = interval(_random); 
        bf2 = interval(_random);
    ...
    fprintf(pipe, "plot [-20 : 20] [-20 : 20] %lf*x**4 + %lf*x**3 + %lf*x**2 + %lf*x**1 + %lf*x**0 , '-' using 1:2 with points\n", 
    x.table[4][0], x.table[3][0], x.table[2][0], x.table[1][0], x.table[0][0]);

    for (auto elem: vec) fprintf(pipe, "%d\t%d\n", elem.first, elem.second);
